(function () {
  ...
  function renderSidebar() {
    ...
    active.forEach(function (mod) {
      html.push('<button class="zerp-sidebar-item" data-module="' + escapeHtml(mod.id) + '">' + escapeHtml(mod.label) + '</button>');
    });
    future.forEach(function (mod) {
      html.push('<button class="zerp-sidebar-item is-disabled" data-disabled="1" title="W produkcji">' + escapeHtml(mod.label) + '</button>');
    });
  ...
  function doSwitch(targetModule) {
    ...
    el.moduleLabel.textContent = labelMap[targetModule] || 'Moduł';
    ...
    el.moduleView.innerHTML = '<div class="zerp-card"><h3>Moduł niedostępny</h3></div>';
  ...
  function moduleDashboard() {
    ...
    wrap.innerHTML = '' +
      '<div class="zerp-summary-strip" id="zerp-summary-strip"></div>' +
      '<div class="zerp-dashboard-grid" id="zerp-dashboard-grid"></div>';
    el.moduleView.innerHTML = '';
    ...
    (state.modules.active || []).forEach(function (mod) {
      ...
      tile.className = 'zerp-tile';
      tile.innerHTML = '<div class="zerp-tile-title">' + escapeHtml(mod.label) + '</div><div class="zerp-muted">' + escapeHtml(mod.description || '') + '</div>';
      tile.addEventListener('click', function () { switchModule(mod.id); });
      tileContainer.appendChild(tile);
    });

    (state.modules.future || []).forEach(function (mod) {
      const tile = document.createElement('div');
      tile.className = 'zerp-tile is-disabled';
      tile.innerHTML = '<div class="zerp-tile-title">' + escapeHtml(mod.label) + '</div><div class="zerp-muted">W produkcji</div>';
      tileContainer.appendChild(tile);
    });
    ...
    api('app/summary').then(function (result) {
      ...
      summaryContainer.innerHTML = '' +
        '<div class="zerp-summary-item"><div class="zerp-muted">Nieprzeczytane powiadomienia</div><div class="zerp-summary-value">' + (s.unread_notifications || 0) + '</div></div>' +
        '<div class="zerp-summary-item"><div class="zerp-muted">Nieprzeczytane rozmowy</div><div class="zerp-summary-value">' + (s.unread_threads || 0) + '</div></div>' +
        '<div class="zerp-summary-item"><div class="zerp-muted">Prośby o dołączenie</div><div class="zerp-summary-value">' + (s.pending_join_requests || 0) + '</div></div>';
    }).catch(function () {
  ...
  function moduleCompanyUsers() {
    ...
    el.moduleView.innerHTML = '' +
      ...
      '  <div class="zerp-card" id="zerp-company-card"><h3 style="margin-top:0">Dane firmy</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '  <div class="zerp-card" id="zerp-relations-card"><h3 style="margin-top:0">Relacje A↔B</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '</div>' +
      '<div class="zerp-grid-two" style="margin-top:12px">' +
      '  <div class="zerp-card" id="zerp-members-card"><h3 style="margin-top:0">Użytkownicy firmy</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '  <div class="zerp-card" id="zerp-joins-card"><h3 style="margin-top:0">Wnioski o dołączenie</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '</div>';
  ...
  function moduleCatalog() {
    ...
    el.moduleView.innerHTML = '' +
      '<div class="zerp-grid-two">' +
      '  <div class="zerp-card" id="zerp-source-card"><h3 style="margin-top:0">Źródło Google Sheets</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '  <div class="zerp-card" id="zerp-local-catalog-card"><h3 style="margin-top:0">Katalog lokalny</h3><div class="zerp-muted">Ładowanie...</div></div>' +
      '</div>' +
      '<div class="zerp-card" id="zerp-merged-products-card" style="margin-top:12px"><h3 style="margin-top:0">Scalona lista produktów</h3><div class="zerp-muted">Ładowanie...</div></div>';
  ...
  function moduleNotifications() {
    ...
    async function render() {
      ...
      el.moduleView.innerHTML = '' +
        '<div class="zerp-card">' +
        '  <div class="zerp-row" style="margin-bottom:10px"><h3 style="margin:0">Powiadomienia</h3><div class="zerp-actions"><button class="zerp-btn" id="zerp-notif-refresh">Odśwież</button><button class="zerp-btn zerp-btn-primary" id="zerp-notif-mark-read" ' + (unread.length ? '' : 'disabled') + '>Oznacz nieprzeczytane jako przeczytane</button></div></div>' +
        '  <div class="zerp-list">' +
        (items.map(function (item) {
          return '<div class="zerp-list-item">' +
            ...
            '<div style="margin-top:6px">' + escapeHtml(item.body || '') + '</div>' +
          '</div>';
        }).join('') || '<div class="zerp-list-item">Brak powiadomień.</div>') +
        '  </div>' +
        '</div>';
  ...
  function bindTopbarActions() {
    ...
    if (el.btnCommunicator) {
      el.btnCommunicator.addEventListener('click', function () {
        ...
        if (!has) {
          mountModal('Komunikator', '<p>Moduł komunikatora nie jest dostępny dla Twojego konta.</p>', [{ label: 'OK' }]);
          return;
        }
