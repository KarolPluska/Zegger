...
final class ZERP_Legacy_Compat
{
    ...
    private static function should_boot_legacy(): bool
    {
        if (!empty($_GET['zq_offer_panel'])) {
            return true;
        }

        $rest_route = '';
        if (isset($_REQUEST['rest_route'])) {
            $rest_route = (string) wp_unslash($_REQUEST['rest_route']);
        }
        if ($rest_route !== '' && strpos($rest_route, '/zq-offer/v1/') !== false) {
            return true;
        }

        $uri = isset($_SERVER['REQUEST_URI']) ? (string) $_SERVER['REQUEST_URI'] : '';
        if ($uri !== '' && strpos($uri, '/wp-json/zq-offer/v1/') !== false) {
            return true;
        }
